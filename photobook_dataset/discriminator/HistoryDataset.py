...
class HistoryDataset(Dataset):
    def __init__(self, split, data_dir, chain_file, segment_file,
        ...
        # Img2chain gives the segments per target and gameid
        self.img2chain = defaultdict(dict)
        for chain in self.chains:
            # TODO: HIer gaat het goed
        ...
        for chain in self.chains:
            ...
            for s in range(len(chain_segments)):
                ...
                if segment_id not in segments_added:
                    ...
                    # Get segment image set and target
                    images = cur_segment_obj['image_set']
                    targets = cur_segment_obj['targets']

                    # For every image in the image set of this segment
                    prev_lengths = defaultdict(int)
                    for im in images:

                        if game_id in self.img2chain[im]:
                            # Gets segment for this image and gameid
                            temp_chain = self.img2chain[im][game_id]
                            # Only keeps the segments that occurred before the current segment_id
                            hist_segments = [t for t in temp_chain if t < segment_id]

                            # Puts concatenated history segments to prev_chains
                        ...
                        # Add length of the concatenated history 'segment'
